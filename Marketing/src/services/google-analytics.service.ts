...
export class GoogleAnalyticsService {
  ...
  private readonly propertyId: number = 253437596;
  constructor(
  ) {
  ...
  async getItemsGA4(query: any, dimensions: Dimension[], metrics: any, orderBys): Promise<any> {
    try {
  ...
  async getDataGA4(query: any, dimensions: Dimension[]): Promise<any> {
    try {
      const [response] = await this.analyticsDataClient.runReport({
        property: `properties/${this.propertyId}`,
        dateRanges: [
  ...
  async handleApiGA4(query: any, producs: any, channelFlag: string): Promise<any> {
    ...
    const ga4 = await this.getItemsGA4(query, dimensions, metrics, orderBys)
    const data: any = []
    await ga4.rows.forEach((e: any, index: any) => {
      ...
      let item = {
        ...
        itemGifts: [],
        itemVouchers: [],
        itemChannelDetail: channel.ga4web,
  ...
  async handleCampain(query: any): Promise<any> {
    ...
    const ga4GoogleAdsCampaign = await this.getItemsGA4(query, dimensionsGoogleAdsCampaign, metricsGoogleAdsCampaign, orderBys)
    ...
    const ga4SessionCampaign = await this.getItemsGA4(query, dimensionsSessionCampaign, metricsSessionCampaign, orderBys);
